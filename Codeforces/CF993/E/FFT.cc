...
template<typename T>
vector<T> multiply(const vector<T> & a, const vector<T> & b, LL mod, bool split = false) {
	...
	//FOR(i,0,ans.size())err=max(err,abs(done[i].x-ans[i]));
		...
		for(int s = 0; s < 3; ++s, mul *= M) {
			...
			FFT(prod, -1);
			FOR(i, 0, ans.size()) ans[i]+= (llround(prod[i].x)%mod)*mul, ans[i]%=mod;
                          //ans[i] += llround(prod[i].x)*mul;
		}
	}
	return ans;
}
